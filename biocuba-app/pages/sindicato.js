...
export default function Sindicato() {
  ...
  async function cargarDatos(s){
    setLoading(true)
    const {data} = await supabase.from('sindicato_ventas').select('*').eq('mes',mes()).eq('sucursal_id',s.sucursal).order('created_at',{ascending:false})
    setVentas(data||[])
    setLoading(false)
  }
  ...
  return (
    <>
      <Head><title>Sindicato Municipal — BioCuba</title></Head>
      <header style={{background:'#fff',borderBottom:'2.5px solid var(--br)',padding:'0 20px',display:'flex',alignItems:'center',minHeight:54,gap:12,flexWrap:'wrap'}}>
        ...
        <span style={{fontSize:13,fontWeight:600,color:'var(--amber)'}}>🤝 Sindicato Municipal</span>
      ...
      <main style={{padding:20,maxWidth:960,margin:'0 auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:20}}>
          {[
            {lbl:'Total del mes',val:fmt(tMes),color:'var(--amber)'},
            {lbl:'Ventas registradas',val:ventas.length,color:'var(--blue)'},
          ].map((k,i)=>(
            <div key={i} style={{background:'#fff',border:'1px solid var(--bdr)',borderRadius:10,padding:'14px 16px'}}>
              ...
              <div style={{fontFamily:'var(--mono)',fontSize:24,fontWeight:700,color:k.color}}>{k.val}</div>
            </div>
          ))}
        </div>
        <div style={{background:'#fff',border:'1px solid var(--bdr)',borderRadius:12,overflow:'hidden'}}>
          <div style={{padding:'14px 16px',borderBottom:'1px solid var(--bdr)',fontSize:13,fontWeight:600}}>
            Ventas del mes — {new Date().toLocaleString('es-CL',{month:'long',year:'numeric'})}
          </div>
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead><tr style={{background:'var(--s2)'}}>
                {['Fecha','Nombre','RUT','Folio','Monto','Obs','Registrado por'].map(h=>(
